...
RcppExport SEXP PO_MPT(SEXP nburn_, SEXP nsave_, SEXP nskip_, SEXP ndisplay_, SEXP ltr_, SEXP subjecti_,
  ...
  for (int iscan=0; iscan<nscan; iscan++){
    ...
    //////////////////////////////////////////////
    if(MPT){
      POloglik(t1, t2, ltr, type, theta[0], theta[1], probs, maxL, MPT, dist, Xbeta+vn, llold);
      ...
      Ys_to_probs(Ys, probs, maxL);
      POloglik(t1, t2, ltr, type, theta[0], theta[1], probs, maxL, MPT, dist, Xbeta+vn, llnew);
      for(int i=0; i<nYs; ++i){
    ...
    if(frailty==1){
      for(int i=0; i<m; ++i){
        ...
        double viold = v[i];
        POloglikblocki(t1, t2, ltr, type, theta[0], theta[1], probs, maxL, MPT, dist, Xbeta, llold, ind1, ind2, v[i]);
        llold += -0.5*D[i]*lambda*std::pow(v[i]-meanvi,2);
        v[i] = Rf_rnorm(viold, sdvi);
        POloglikblocki(t1, t2, ltr, type, theta[0], theta[1], probs, maxL, MPT, dist, Xbeta, llnew, ind1, ind2, v[i]);
    ...
    if(iscan>=nburn){
      ...
      if(skiptally>nskip){
        ...
        Linv.col(isave) = POinvLik(t1, t2, ltr, type, theta[0], theta[1], probs, maxL, MPT, dist, Xbeta+vn);
        // calculate -2loglikelihood
        double tempLik = 0;
        POloglik(t1, t2, ltr, type, theta[0], theta[1], probs, maxL, MPT, dist, Xbeta+vn, tempLik);
  ...
  double Dmean = 0;
  POloglik(t1, t2, ltr, type, sumtheta[0]/(nsave+0.0), sumtheta[1]/(nsave+0.0), 
...
RcppExport SEXP PO_MPT_plots(SEXP tgrid_, SEXP xpred_, SEXP theta_, SEXP beta_, SEXP Ys_, 
  ...
  for(int i=0; i<nsave; ++i){
    ...
    for(int j=0; j<npred; ++j){
      for(int k=0; k<ngrid; ++k){
        estf(k, i, j) = std::exp(POlogpdf(tgrid[k], theta(0,i), theta(1,i), probs, maxL, true, dist,
                                 xbeta[j]));
        estS(k, i, j) = std::exp(POlogsurv(tgrid[k], theta(0,i), theta(1,i), probs, maxL, true, dist,
                                 xbeta[j]));
