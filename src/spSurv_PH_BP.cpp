...
RcppExport SEXP PH_BP(SEXP nburn_, SEXP nsave_, SEXP nskip_, SEXP ndisplay_, SEXP ltr_, SEXP subjecti_,
  ...
  for (int iscan=0; iscan<nscan; iscan++){
    ...
    if(frailty==1){// CAR frailty
      for(int i=0; i<m; ++i){
        ...
        double viold = v[i];
        PH_BP_loglikblocki(t1, t2, ltr, type, theta[0], theta[1], weight, BP, dist, Xbeta, llold, ind1, ind2, v[i]);
        llold += -0.5*D[i]*lambda*std::pow(v[i]-meanvi,2);
        v[i] = Rf_rnorm(viold, sdvi);
        PH_BP_loglikblocki(t1, t2, ltr, type, theta[0], theta[1], weight, BP, dist, Xbeta, llnew, ind1, ind2, v[i]);
    ...
    if(iscan>=nburn){
      ...
      if(skiptally>nskip){
        ...
        logLik.col(isave) = PH_BP_logliki(t1, t2, ltr, type, theta[0], theta[1], weight, BP, dist, Xbeta+vn);
        // calculate -2loglikelihood
        double tempLik = 0;
        PH_BP_loglik(t1, t2, ltr, type, theta[0], theta[1], weight, BP, dist, Xbeta+vn, tempLik);
        ...
        for(int i=0; i<nsub; ++i){
          ...
          for(int j=ind1; j<=ind2; ++j){
            ...
            if((type[j]==0)||(type[j]==1)){
              ...
              double logtmp = PH_BP_logsurv(t2[j], theta[0], theta[1], weight, BP, dist, Xbeta[j]+vn[j]);
              logSt1tmp += logtmp; logSt1tmp += logSt0tmp;
              logSt2tmp += logtmp; logSt2tmp += logSt0tmp;
            }else{
              double logtmp1 = PH_BP_logsurv(t1[j], theta[0], theta[1], weight, BP, dist, Xbeta[j]+vn[j]);
              double logtmp2 = PH_BP_logsurv(t2[j], theta[0], theta[1], weight, BP, dist, Xbeta[j]+vn[j]);
  ...
  double Dmean = 0;
  PH_BP_loglik(t1, t2, ltr, type, sumtheta[0]/(nsave+0.0), sumtheta[1]/(nsave+0.0), 
...
// Get LOO and WAIC
RcppExport SEXP PH_BP_loo_waic(SEXP ltr_, SEXP subjecti_, SEXP t1_, SEXP t2_, SEXP type_, 
  ...
  for(int isave=0; isave<nsave; ++isave){
    ...
    // calculate loglikelihood
    logLik.col(isave) = PH_BP_logliki(t1, t2, ltr, type, th1, th2, wi, true, dist, xbeta);
  }
...
// Get Cox-Snell residuals
RcppExport SEXP PH_BP_cox_snell(SEXP ltr_, SEXP subjecti_, SEXP t1_, SEXP t2_, SEXP type_, 
                                SEXP X_, SEXP theta_, SEXP beta_, SEXP vn_, SEXP weight_, SEXP dist_){
  BEGIN_RCPP
  // Transfer R variables into C++;
  const Rcpp::NumericVector ltr(ltr_); // n by 1;
    ...
    for(int i=0; i<nsub; ++i){
      ...
      for(int j=ind1; j<=ind2; ++j){
        ...
        if((type[j]==0)||(type[j]==1)){
          ...
          double logtmp = PH_BP_logsurv(t2[j], th1, th2, wi, true, dist, xbeta[j]);
          logSt1tmp += logtmp; logSt1tmp += logSt0tmp;
          logSt2tmp += logtmp; logSt2tmp += logSt0tmp;
        }else{
          double logtmp1 = PH_BP_logsurv(t1[j], th1, th2, wi, true, dist, xbeta[j]);
          double logtmp2 = PH_BP_logsurv(t2[j], th1, th2, wi, true, dist, xbeta[j]);
          logSt1tmp += logtmp1; logSt1tmp += logSt0tmp;
          logSt2tmp += logtmp2; logSt2tmp += logSt0tmp;
        }
      }
...
RcppExport SEXP PH_BP_plots(SEXP tgrid_, SEXP xpred_, SEXP theta_, SEXP beta_, SEXP v_,
  ...
  for(int i=0; i<nsave; ++i){
    ...
    for(int j=0; j<npred; ++j){
      for(int k=0; k<ngrid; ++k){
        estf(k, i, j) = std::exp(PH_BP_logpdf(tgrid[k], theta(0,i), theta(1,i), wi, true, dist,
                                 xbeta[j]));
        estS(k, i, j) = std::exp(PH_BP_logsurv(tgrid[k], theta(0,i), theta(1,i), wi, true, dist,
                                 xbeta[j]));
