...
RcppExport SEXP POloglogistic( SEXP nburn_, SEXP nsave_, SEXP nskip_, SEXP ndisplay_,
  ...
  Xbeta_r = X_r*beta_r;
    ...
    //////////////////////////////////////////////
    if(cpar<R_PosInf){
      POloglik(t1, t2, type, Xbeta, probs, maxL, theta[0], theta[1], llold);
      ...
      Ys_to_probs(Ys, probs, maxL);
      POloglik(t1, t2, type, Xbeta, probs, maxL, theta[0], theta[1], llnew);
      for(int i=0; i<nYs; ++i){
    ...
    if(iscan>=nburn){
        ...
        Linv.col(isave) = POinvLik(t1, t2, type, Xbeta, probs, maxL, theta[0], theta[1]);
        // calculate -2loglikelihood
        double tempLik = 0;
        POloglik(t1, t2, type, Xbeta, probs, maxL, theta[0], theta[1], tempLik);
  ...
  double Dmean = 0;
  POloglik(t1, t2, type, Xbeta, probs, maxL, sumtheta[0]/(nsave+0.0), sumtheta[1]/(nsave+0.0), Dmean);
...
RcppExport SEXP frailtyPOloglogistic( SEXP nburn_, SEXP nsave_, SEXP nskip_, SEXP ndisplay_,
    SEXP t1_, SEXP t2_, SEXP type_, SEXP X_, SEXP theta_, SEXP beta_, SEXP cpar_, SEXP Ys_, SEXP maxL_,
    SEXP a0_, SEXP b0_, SEXP theta0_, SEXP V0inv_,SEXP Vhat_, SEXP beta0_, SEXP S0inv_, SEXP Shat_, 
  ...
  Xbeta_r = X_r*beta_r;
    ...
    //////////////////////////////////////////////
    if(cpar<R_PosInf){
      POloglik(t1, t2, type, Xbeta+vn, probs, maxL, theta[0], theta[1], llold);
      tmpold = arma::log(Ys_r/(1.0-Ys_r));
      for(int i=0; i<nprob; ++i) probsold[i] = probs[i];
      if(iscan>l0){
        tmpnew = mvrnorm(tmpold, YsSnew);
      ...
      Ys_r = 1.0/(1.0+arma::exp(-tmpnew));
      Ys_to_probs(Ys, probs, maxL);
      POloglik(t1, t2, type, Xbeta+vn, probs, maxL, theta[0], theta[1], llnew);
      for(int i=0; i<nYs; ++i){
        j2 = std::pow((int)(log(i+1)/log(2))+1, 2);
        llold += cpar*j2*tmpold[i]-2.0*cpar*j2*log(1+exp(tmpold[i]));
        llnew += cpar*j2*tmpnew[i]-2.0*cpar*j2*log(1+exp(tmpnew[i]));
    ...
    if(car){
      for(int i=0; i<m; ++i){
        ...
        POloglikblocki(t1, t2, type, Xbeta, probs, maxL, theta[0], theta[1], llold, ind1, ind2, v[i]);
        viold = v[i];
        v[i] = Rf_rnorm(meanvi, std::sqrt(1.0/(lambda*D[i])));
        POloglikblocki(t1, t2, type, Xbeta, probs, maxL, theta[0], theta[1], llnew, ind1, ind2, v[i]);
        ratio = exp(llnew-llold);
        uu = unif_rand();
        if(uu>ratio){
          v[i]=viold;
    ...
    if(iscan>=nburn){
  	  ...
  	  if(skiptally>nskip){
        // calculate 1.0/likelihood
        Linv.col(isave) = POinvLik(t1, t2, type, Xbeta+vn, probs, maxL, theta[0], theta[1]);
        // calculate -2loglikelihood
        double tempLik = 0;
        POloglik(t1, t2, type, Xbeta+vn, probs, maxL, theta[0], theta[1], tempLik);
        Dvalues(isave) = -2.0*tempLik;
    		// save regression coefficient
        theta_save(_,isave) = theta;
        sumtheta += theta; 
  ...
  Ys_to_probs(sumYs/(nsave+0.0), probs, maxL);
  double Dmean = 0;
  POloglik(t1, t2, type, Xbeta+sumvn/(nsave+0.0), probs, maxL, sumtheta[0]/(nsave+0.0), sumtheta[1]/(nsave+0.0), Dmean);
  double pD = meanD + 2.0*Dmean;
  double DIC = meanD + pD; 
...
RcppExport SEXP POloglogistic_plots(SEXP tgrid_, SEXP xpred_, SEXP theta_, SEXP beta_, SEXP Ys_, SEXP maxL_, SEXP CI_){
  BEGIN_RCPP
  // Transfer R variables into C++;
  ...
  for(int i=0; i<nsave; ++i){
    ...
    for(int j=0; j<npred; ++j){
      for(int k=0; k<ngrid; ++k){
        estf(k, i, j) = std::exp(POlogpdf(tgrid[k], xbeta[j], probs, maxL, theta(0,i), theta(1,i)));
        estS(k, i, j) = POSurv(tgrid[k], xbeta[j], probs, maxL, theta(0,i), theta(1,i));
